package com.acromere.aveon.elementary;

import com.acromere.aveon.ElementaryFlow;
import com.acromere.curve.math.Geometry;
import com.acromere.curve.math.Vector;

public class VortexSheetFlow implements ElementaryFlow {
	...
	public VortexSheetFlow( double x1, double y1, double x2, double y2, double q ) {
		this.ax = x1;
		this.ay = y1;
		this.bx = x2;
		this.by = y2;
	...
	public double[] getVelocity( double xf, double yf ) {
		...
		if( ar2 > 0.0 && br2 > 0.0 ) {
			...
			if( alpha < -Math.PI ) alpha += 2.0 * Math.PI;
			velocity[ 0 ] = q * alpha * cos + q * Math.log( ar2 / br2 ) / 2.0 * sin;
			velocity[ 1 ] = -q * Math.log( ar2 / br2 ) / 2.0 * cos + q * alpha * sin;
		}
